    ...
    for address in "${ipaddresses[@]}"
         do
	      echo -e "\e[96m Starting process for $address: \e[39m"
            sshpass -ev rsync -avz -e ssh --progress executables.tar.gz remotenosu.sh workerstart.sh sliceworker.service $address:
            sshpass -ev ssh -t -o "StrictHostKeyChecking=no"  $address "echo $sspass1 |  -S ./remotenosu.sh"
	    
            insed="echo $sspass1 |  -S sed -i 's/{MASTER_IP}/$masterip/g' /opt/sliceup/executables/flink-1.10.0/conf/flink-conf.yaml"
	        sshpass -ev ssh -t -o "StrictHostKeyChecking=no"  $address "$insed"  

            insed="echo $sspass1 |  -S sed -i 's/{MASTER_IP}/$masterip/g' /opt/sliceup/executables/conf.ini"
	        sshpass -ev ssh -t -o "StrictHostKeyChecking=no"  $address "$insed"  

            insed="echo $sspass1 |  -S sed -i 's/{WORKER_IP}/$address/g' /opt/sliceup/executables/conf.ini"
            sshpass -ev ssh -t -o "StrictHostKeyChecking=no"  $address "$insed"  

            sshpass -ev ssh -o "StrictHostKeyChecking=no" $address "echo $sspass1 |  -S systemctl start sliceworker.service"
